...
public interface LeaderboardRepository extends JpaRepository<UserEarnedPoints, Integer> {
    ...
    @Query(value = """
            SELECT new com.ogreten.catan.leaderboard.dto.UserWithPoints(new com.ogreten.catan.auth.domain.User(u.id, u.email, u.password, u.firstName, u.lastName), SUM(uep.points) as points)
            FROM User u
